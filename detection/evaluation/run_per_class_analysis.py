...
def analyze_all_results():
    ...
    for model_folder in folders:
        ...
        for filename in tqdm(filenames, desc=f"Models in {model_folder}"):
            ...
            image_class_metrics = calculate_per_class_image_metrics(results, CONF_THRESHOLDS)
            ...
            for cls_id, cls_name in CLASS_NAMES.items():
                ...
                if max_recall > 0:
                    ...
                    candidates.sort(key=lambda x: (x["specificity"], x["threshold"]), reverse=True)
                    best_m = candidates[0]
    ...
    if all_summary_rows:
        ...
        highlights = summary_df[(summary_df["cycle"] == 4) & (summary_df["dataset"] == "test") & (summary_df["class_id"] == 2)]
        print(highlights[["model", "processing", "AP", "recall_0.1", "specificity_0.1", "best_thresh", "spec_at_best_thresh"]].to_string(index=False))
