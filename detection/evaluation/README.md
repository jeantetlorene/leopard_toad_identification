...
- **[`evaluation_suite.py`](evaluation_suite.py)**: Comprehensive metric calculator. Aggregates cached prediction JSONs to output threshold sweeps, ROC-AUCs, and unified comparison CSVs. It automatically routes datasets for mathematically correct evaluation: full unlabelled sequences (`_full_seq_raw.json`) are exclusively used for image-level binary filtering metrics (ROC-AUC), while annotated subsets (`_raw.json`) are exclusively used for detection-level metrics (mAP). It also dynamically calculates optimal per-class thresholds on the validation set to evaluate "before and after" deployment performance on the test set. Can be run independently with filtering arguments (`--models`, `--cycles`, `--processing`, `--variants`).
