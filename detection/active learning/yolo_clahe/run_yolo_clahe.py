...
from config import BASE_DIR, PRETRAINED_WEIGHTS, SCRATCH_WEIGHTS, YOLO_DIR
from trainer_clahe import train_phase_1, train_phase_2, train_scratch


def run_training_for_mode(mode):
    print(f"\n{'=' * 50}\nStarting YOLO CLAHE Training [{mode.upper()}]\n{'=' * 50}")

    data_dir = os.path.join(BASE_DIR, "active learning", "data", "yolo", mode)
    ...
    for cycle_folder in cycles:
        ...
        print(f"\n>> Cycle {cycle_num}: Training YOLO Model with CLAHE.")

        cycle_dir = os.path.join(YOLO_DIR, "cycles", mode, f"cycle_{cycle_num}")
