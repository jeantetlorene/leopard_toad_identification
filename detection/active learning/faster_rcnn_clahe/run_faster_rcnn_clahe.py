...
from config import BASE_DIR, PRETRAINED_WEIGHTS, SCRATCH_WEIGHTS, FASTER_RCNN_DIR
from trainer_clahe import train_phase_1, train_phase_2, train_scratch


def run_training_for_mode(mode):
    print(
        f"\n{'=' * 50}\nStarting Faster R-CNN CLAHE Training [{mode.upper()}]\n{'=' * 50}"
    )

    data_dir = os.path.join(BASE_DIR, "active learning", "data", "faster_rcnn", mode)
    ...
    for cycle_folder in cycles:
        ...
        print(f"\n>> Cycle {cycle_num}: Training Faster R-CNN Model with CLAHE.")

        cycle_dir = os.path.join(FASTER_RCNN_DIR, "cycles", mode, f"cycle_{cycle_num}")
        ...
        if mode == "pretrained":
            ...
            if os.path.exists(expected_p2_model):
                ...
                p2 = train_phase_2(
                    p1, f"cycle_{cycle_num}_pretrained", dataset_dir, epochs=30
                )
        else:
