...
def extract_metadata(csv_path):
    ...
    mode = "pretrained" if "pretrained" in filename else "scratch" if "scratch" in filename else "unknown"
...
def main():
    parser = argparse.ArgumentParser(description="Prepare images for annotation by copying them to a Downloads folder.")
    parser.add_argument("csv_path", type=str, help="Path to the query candidates CSV file.")
    ...
    if dest_dir.exists():
        print(f"!! Warning: Directory {dest_dir} already exists. Images will be added/overwritten.")
    else:
