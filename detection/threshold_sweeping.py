...
for img in valid_images:
    ...
    for cls in classes:
        ...
        is_positive = (
            (df_img["class_name"] == cls) & (df_img["evaluation"] == "Correct")
        ).any()
...
for model in MODELS:
    ...
    if len(model_dfs) > 0:
        ...
        img_level_preds = (
            model_all_preds.groupby(["image_path", "class_name"])["confidence"]
            .max()
            .reset_index()
        )
...
all_preds_df = (
    pd.concat(pred_records, ignore_index=True) if pred_records else pd.DataFrame()
)
...
if not all_preds_df.empty:
    all_preds_df["model"] = (
        all_preds_df["model"].map(MODEL_MAPPING).fillna(all_preds_df["model"])
    )
...
master_df = pd.merge(
    master_df, all_preds_df, on=["model", "class_name", "image_path"], how="left"
)
...
for year in years_to_evaluate:
    for model in MODELS:
        for cls in classes:
            ...
            if year == "All Years":
                df_cut = master_df[
                    (master_df["model"] == model) & (master_df["class_name"] == cls)
                ]
            else:
                df_cut = master_df[
                    (master_df["model"] == model)
                    & (master_df["class_name"] == cls)
                    & (master_df["year"] == year)
                ]
            ...
            optimal_thresholds.append(
                {
                    ...
                    "Class": cls,
                    "Model": model,
                    f"Threshold_at_{int(target_recall * 100)}%_Recall": round(
                        best_cut["threshold"], 2
                    ),
...
def plot_evaluation_curves(metrics, var_x, var_y, title_template, ylabel):
    ...
    for year in unique_years:
        for cls in unique_classes:
            data_cut = metrics[
                (metrics["year"] == year) & (metrics["class_name"] == cls)
            ]
            if data_cut.empty:
                continue
            ...
            plt.xlabel(
                "Threshold" if var_x == "threshold" else var_x.capitalize(), size=12
            )
...
plot_evaluation_curves(
    metrics_df, "threshold", "recall", "Recall vs Threshold", "Image-Level Recall"
)
...
plot_evaluation_curves(
    ...
    "threshold",
    "fp_per_1000",
    "FP per 1000 vs Threshold",
    "False Positives per 1000 Images",
)
...
for year in years_to_evaluate:
    for cls in classes:
        data_cut = metrics_df[
            (metrics_df["year"] == year) & (metrics_df["class_name"] == cls)
        ]
        if data_cut.empty:
            continue
...
for cls in classes:
    for model in MODELS:
        df_cut = master_df[
            (master_df["class_name"] == cls) & (master_df["model"] == model)
        ]
        if df_cut.empty:
            continue
        ...
        plt.title(
            f"TP vs FP Density Overlay - {model} | {cls} (All Years)",
            weight="bold",
            size=14,
        )
...
for model in MODELS:
    ...
    plt.title(
        f"TP vs FP Density Overlay - {model} | All Classes Combined (All Years)",
        weight="bold",
        size=14,
    )
    plt.xlabel("Confidence", size=12)
