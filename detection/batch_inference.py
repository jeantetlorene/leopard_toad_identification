...
def process_folder(input_folder, output_folder, model, conf_threshold, img_size, batch_size, device, model_type="yolo"):
            ...
            with tqdm(total=len(images), desc=f"Processing {target_dir.name}") as pbar:
                for i in range(0, len(images), batch_size):
                    ...
                    if model_type == "yolo":
                        ...
                        batch_tensors = [TF.to_tensor(img).to(device) for img in batch_input_imgs]
                        with torch.no_grad():
                    ...
                    for img_path, result in zip(valid_img_paths, batch_results):
                        ...
                        if model_type == "yolo":
                            ...
                            faster_rcnn_names = {1: "Other_Amphibian", 2: "Small_Mammal", 3: "Western_Leopard_Toad"}
                            ...
                            for k in range(len(pred_scores)):
                                ...
                                if conf >= conf_threshold:
                                    ...
                                    class_name = faster_rcnn_names.get(cls_id, f"Unknown_{cls_id}")
...
def main():
    ...
    folders = [
        '01.09.09.09.2025', '06.10-13.10.2025', '15.09-22.09.2025', '25.08-01.09.2025',
        '04.08.2025', '09.09.15.09.2025', '21-28.08.2025', '29.09.06.10.2025',
        '05.08.2025', '12.19.08.2025', '22.09-29.09.2025',
        "Cameras - AI Data", "Cameras-Master Data",
        "02.09.2024", "09.02.2024", "16.09.2024", "19.09.2024", "26.09.2024",
        "02.10.2024", "11.10.2024", "18.11.2024", "23.08.2024", "28.08.2024"
    ]
    ...
    for model_info in models_config:
        ...
        if m_type == "yolo":
            ...
            eval_device = torch.device(f"cuda:{DEVICE}" if torch.cuda.is_available() else "cpu")
        ...
        for year, base_input_dir in years.items():
            ...
            for folder in folders:
                ...
                processed = process_folder(in_dir, out_dir, model, CONF_THRESHOLD, IMG_SIZE, BATCH_SIZE, eval_device, m_type)
