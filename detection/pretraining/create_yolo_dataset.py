...
def build_cat_map(categories):
    ...
    for cat in categories:
        ...
        if 'frog' in name or 'toad' in name:
            cat_map[cat['id']] = 'Frog'
        elif ('mouse' in name or 'rat' in name or 'muskrat' in name) and ('snake' not in name):
...
def main():
    ...
    ohio_json_path = os.path.join(base_dir, 'ohio_small_animals_subset', 'filtered_annotations.json')
    cali_json_path = os.path.join(base_dir, 'california_small_animals_subset', 'filtered_annotations.json')
    ...
    print(f"Done. Processed {cali_count} California images and {ohio_count} Ohio images.")
