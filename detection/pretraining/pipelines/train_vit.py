    ...
    def __getitem__(self, idx):
        ...
        if self.transform and len(boxes) > 0:
            transformed = self.transform(
                image=image, bboxes=boxes, class_labels=classes
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Fine-tune a SOTA ViT Model (YOLOS) on a YOLO-format dataset."
    )
    parser.add_argument(
        "--data_yaml",
        type=str,
        default="/home/Joshua/Downloads/leopard_toad_identification/dataset/dataset/data.yaml",
    ...
    parser.add_argument(
        ...
        help="Hugging Face ViT model name (e.g., hustvl/yolos-small, hustvl/yolos-base)",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="./yolos_output",
