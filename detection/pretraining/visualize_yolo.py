...
def load_image_with_bboxes(img_idx, images_dir, labels_dir):
            ...
            for line in f:
                ...
                if len(parts) >= 5:
                    ...
                    try:
                        ...
                        bboxes_info.append(
                            f"{label}: [{xmin:.1f}, {ymin:.1f}, {xmax:.1f}, {ymax:.1f}]"
                        )
                    except ValueError:
                        continue
    else:
    ...
    status = (
        f"**Image {img_idx + 1} / {len(image_files)}** - `{img_name}`\n\n"
        + "\n".join(bboxes_info)
    )
    return img, status
...
with gr.Blocks(title="YOLO Dataset Viewer") as demo:
    ...
    with gr.Row():
        images_dir_input = gr.Textbox(
            ...
            value="/home/Joshua/Downloads/leopard_toad_identification/dataset/dataset1/images",
        )
        labels_dir_input = gr.Textbox(
            label="Labels Directory",
            value="/home/Joshua/Downloads/leopard_toad_identification/dataset/dataset1/labels",
        )
