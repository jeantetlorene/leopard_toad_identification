...
def calculate_pr_and_cm(model, data_loader, device, class_names, iou_threshold=0.5, conf_threshold=0.5):
    """
    ...
    pr_data = {c: {'scores': [], 'matches': [], 'num_gt': 0} for c in range(1, num_classes + 1)}
...
def plot_results(pr_data, cm_y_true, cm_y_pred, class_names, save_dir):
    ...
    if len(all_py) > 0:
        ...
        ax.plot(px, mean_py, label=f"all classes {mean_ap:.3f} mAP@0.5", color='blue', linewidth=3)
    ...
    plt.savefig(os.path.join(save_dir, "PR_curve_fixed.png"), dpi=300, bbox_inches="tight")
    plt.close()
    ...
    plt.grid(False)
    plt.savefig(os.path.join(save_dir, "confusion_matrix_fixed.png"), dpi=300, bbox_inches="tight")
    plt.close()
...
def main():
    ...
    test_loader = DataLoader(YoloToFasterRCNNDataset(dataset_path, 'test'), batch_size=32, shuffle=False, collate_fn=collate_fn)
    val_loader = DataLoader(YoloToFasterRCNNDataset(dataset_path, 'val'), batch_size=32, shuffle=False, collate_fn=collate_fn)
    ...
    pr_data, cm_y_true, cm_y_pred = calculate_pr_and_cm(model, test_loader, device, class_names)
