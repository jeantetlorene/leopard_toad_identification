#!/usr/bin/env python3
"""
Bulk-download Ohio Small Animals images + filtered annotations
for anything matching: toad, rat, or frog.

Examples:
  python download_ohio_small_animals.py

  python download_ohio_small_animals.py \
      --metadata-url "https://storage.googleapis.com/public-datasets-lila/osu-small-animals/osu-small-animals.json.zip" \
      --output-dir ohio_small_animals_subset

  python download_ohio_small_animals.py \
      --metadata-file /path/to/osu-small-animals.json \
      --workers 16 \
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Download Ohio Small Animals images/annotations for toad, rat, or frog."
    )
    parser.add_argument(
        "--metadata-file",
        type=str,
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="/home/Joshua/Downloads/leopard_toad_identification/dataset/ohio_small_animals_subset",
    ...
    parser.add_argument(
        ...
        help="Case-insensitive keywords to match in species/file paths.",
    )
    parser.add_argument(
        "--workers",
        type=int,
...
def filter_dataset(
    ...
    - COCO-like structures with categories/annotations/images
    """
    ...
    # Match by image path/text
    for im in images:
        blob = image_text_blob(im)
        if any(k in blob for k in keywords):
            if "id" in im and isinstance(im["id"], int):
                matched_image_ids.add(im["id"])

    # Match by annotation category if COCO categories are present
    ...
    # Build matched annotations list
    if annotations:
        for ann in annotations:
            if ann.get("image_id") in {im.get("id") for im in matched_images}:
                matched_annotations.append(ann)

    # If metadata is only a flat list of image dicts, support that too
...
def make_filtered_output(
    ...
    # Preserve common top-level fields when present
    ...
    if "categories" in original:
        if matched_category_ids:
            ...
            # Keep all categories if we only matched by file path
...
def main() -> int:
    ...
    # Support either dict-style COCO metadata or a flat list supplied by user
