...
detect_2_dir = "/home/Joshua/Downloads/leopard_toad_identification/detection/results/detect_2"

# Find all evaluation CSV files
eval_files = glob.glob(os.path.join(detect_2_dir, "**/*_evaluations.csv"), recursive=True)
...
#%%
# Combine all data
all_correct_df = pd.concat(correct_dfs, ignore_index=True) if correct_dfs else pd.DataFrame()
all_incorrect_df = pd.concat(incorrect_dfs, ignore_index=True) if incorrect_dfs else pd.DataFrame()
...
class_eval_counts = validation_set_df.groupby(['class_name', 'evaluation']).size().unstack(fill_value=0)
...
year_eval_counts = unique_images_df.groupby(['year', 'evaluation']).size().unstack(fill_value=0)
...
if 'confidence' in validation_set_df.columns:
    conf_stats = validation_set_df.groupby('evaluation')['confidence'].agg(['mean', 'std', 'min', 'max'])
    print(conf_stats)
else:
...
if not correct_only_df.empty:
    correct_yearly_class_stats = correct_only_df.groupby(['year', 'class_name']).size().unstack(fill_value=0)
