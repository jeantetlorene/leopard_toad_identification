...
def load_csv(csv_path):
    ...
    return (
        df,
        0,
        f"Successfully loaded {len(unique_images)} images.",
        evaluations,
        unique_images,
    )
...
with gr.Blocks(theme=gr.themes.Soft()) as demo:
    ...
    next_btn.click(
        next_image, inputs=[index_state, images_state], outputs=[index_state]
    ).then(
        update_ui,
        inputs=[images_state, df_state, index_state, evals_state],
        outputs=[image_output, progress_label, jump_num],
    ...
    prev_btn.click(prev_image, inputs=[index_state], outputs=[index_state]).then(
        update_ui,
        inputs=[images_state, df_state, index_state, evals_state],
        outputs=[image_output, progress_label, jump_num],
    )
    ...
    jump_btn.click(
        jump_to_image, inputs=[jump_num, images_state], outputs=[index_state]
    ).then(
        update_ui,
        inputs=[images_state, df_state, index_state, evals_state],
        outputs=[image_output, progress_label, jump_num],
