...
def process_split(consensus_csv, curation_csv, eval_csv, output_csv):
    ...
    merged = curation_df[curation_df["is_representative"] == True].merge(eval_df, on="image_path", how="left")
    
    # Identify bad clusters
    bad_clusters = merged[merged["evaluation"] == "Incorrect"]["cluster_id"].unique()
    print(f"  Found {len(bad_clusters)} bad clusters out of {curation_df['cluster_id'].nunique()}")

    # Find all predictions belonging to bad clusters
    bad_predictions = curation_df[curation_df["cluster_id"].isin(bad_clusters)]
    print(f"  Dropping {len(bad_predictions)} predictions associated with bad clusters.")
    ...
    def make_id(df):
        return df["image_path"] + "_" + df["xmin"].round(1).astype(str) + "_" + df["ymin"].round(1).astype(str) + "_" + df["xmax"].round(1).astype(str) + "_" + df["ymax"].round(1).astype(str)
    ...
    final_df = consensus_df[~consensus_df["_match_id"].isin(bad_ids)].drop(columns=["_match_id"])
