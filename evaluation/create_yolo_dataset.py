...
def process_split(eval_csv, final_csv, split_name, base_dir, sample_classes_txt):
    ...
    missed_images = eval_df[eval_df['evaluation'] == 'Missed Animal']['image_path'].tolist()
    ...
    print(f"  Found {len(correct_images)} correct images and {len(missed_images)} missed images.")
...
if __name__ == "__main__":
    ...
    if os.path.exists(test_eval) and os.path.exists(test_final):
        process_split(test_eval, test_final, "test", output_base_dir, sample_classes_txt)
    else:
